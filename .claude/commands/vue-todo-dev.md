...
```
VueTodoList/
├── config.yml              # Local runtime config (host/port/adminPassword/mediaPath/collabMode)
├── api/index.ts            # Vercel serverless entry: runMigrations + Express handler
├── vercel.json             # buildCommand + devCommand + rewrites
├── server/src/
│   ├── index.ts            # Local entry: runMigrations → listen; setupWebSocket if collabMode=websocket
│   ├── app.ts              # Express factory: all routes + /api/config + static client/dist in prod
...
│   │   └── migrations.ts   # Schema: users, todos, documents, api_tokens, doc_collaborators
│   ├── models/             # User.ts / Todo.ts / Document.ts / ApiToken.ts — OOP, scoped to user_id
│   ├── controllers/        # AuthController, TodoController, DocumentController, MediaController, AdminController, ExternalController, ApiTokenController, CollabController
│   ├── middleware/
│   │   ├── auth.ts         # JWT Bearer, 7d expiry; signToken()
│   │   └── admin.ts        # x-admin-password header, MD5 compare (case-insensitive)
│   ├── collab/
│   │   └── websocket.ts    # WebSocket server; rooms by docId; broadcasts update+cursor msgs
│   └── routes/             # auth, todos, documents (+ collab subroutes), media, admin, external, apitoken
└── client/src/
    ...
    └── views/              # Login, Register, Home, Editor, Admin, ApiToken, Invite
```

## Run

```bash
yarn dev                  # server :3300, Vite :5173 (proxy /api + /ws → 3300)
...
- Collab on Vercel: always polling (WebSocket not supported); set `collabMode: polling` or omit

## Collaborative Editing

- `collabMode: polling` — frontend polls `/api/documents/:id/collab/poll?since=` every 2.5s (Vercel compatible)
- `collabMode: websocket` — WebSocket at `/ws?token=&docId=`; real-time update+cursor broadcast (local only)
- `collabMode: disabled` — sync disabled entirely
- Frontend toggle: toolbar 🔗 button; state persisted in `localStorage`
- Cursor positions broadcast via WebSocket `{type:'cursor', line, col}`; shown as colored badges in toolbar
- Invite flow: owner generates invite link → `/invite/:token` → recipient logs in and accepts → doc appears in their sidebar

### Collab API (`/api/documents/:id/collab/*` — requires JWT)
...
|--------|-------|------|-------------|
| GET | `/api/collab/invite/:token` | None | Get invite info (docTitle, status) |
| POST | `/api/collab/invite/:token/accept` | JWT | Accept invite; returns `{docId}` |
...
| collabMode | polling | polling \| websocket \| disabled |
...
- Remote update: directly sets textarea value via native setter + `renderPreview()` to avoid cursor reset
...
| Timestamp clock skew | Use JS `Math.floor(Date.now()/1000)` for all timestamps — never `strftime('%s','now')` in SQL |
